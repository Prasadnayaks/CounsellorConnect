...
class _ChatbotOverviewScreenState extends State<ChatbotOverviewScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _allItems = [
        ...
        SelfCareItem(id: 'general', name: 'General Wellness', subtitle: 'Core practices', icon: Icons.spa_outlined, gradientColors: [colorScheme.primary.withOpacity(0.8), colorScheme.primary.withOpacity(0.6)]),
      ...
      setState(() {
        // Separate special items from category items
        _specialItems = _allItems.where((item) => item.id == 'general' || item.id == 'favorites').toList();
        _categoryItems = _allItems.where((item) => item.id != 'general' && item.id != 'favorites').toList();
      });
    });
  }
  ...
  void _navigateToSelfCareItemDetail(BuildContext context, SelfCareItem item) {
    Navigator.push(
      context,
      MaterialPageRoute(
        // Navigate to the renamed and updated detail screen
        builder: (context) => SelfCareItemDetailScreen(item: item), // Pass SelfCareItem
      ),
    );
  }
